...
本文说明当前 pnpm workspace 的职责、依赖方向和构建顺序。应用管理、外部连接器与 CLI 的后续
设计仍以 `docs/spec/2026-07-10-monorepo-application-sdk-architecture.md` 为准。
...
apps/
  ...
  idp-server/      生产部署进程入口
packages/
  server-core/     npm 包 gitea-oidc、认证服务核心和兼容 SDK
```
...
```text
admin-web --构建产物--> server-core <--workspace dependency-- idp-server
                         ^
                         |
                 外部 npm 消费者
```

- `admin-web` 不导入服务端源码，只通过 HTTP API 通信。
- `idp-server` 依赖公开包 `gitea-oidc`，不跨目录导入其源码。
- `server-core` 不依赖 `apps/*` 的运行时代码；装配脚本只在仓库构建阶段读取管理台产物。
...
1. 清理所有 workspace 输出。
2. 构建 `admin-web`。
3. 装配静态资源并构建 `server-core`。
4. 基于已生成的公开声明构建 `idp-server`。
...
`test:pack` 从 `packages/server-core/` 检查实际发布白名单。tarball 必须包含完整静态页面和所有
exports，不得包含源码测试、数据库、Agent 文件或 workspace 配置。
