...
该包目前是 monorepo 内部包，版本为 `0.0.0` 且保持 `private: true`。当前提供 `gitea@1` 和
`gitea@2`，两者都支持目标版本 `1.24`、`1.25` 和 `1.26`。`gitea@1` 用于保持已有快照稳定；新建
应用默认选择 `gitea@2`。
...
| 能力 | Gitea 1.24 | Gitea 1.25 | Gitea 1.26 |
| --- | --- | --- | --- |
| 图标、跳过本地 2FA、Required Claim | 支持 | 支持 | 支持 |
| Group Claim、管理员/受限组、组织团队映射 | 支持 | 支持 | 支持 |
| 全名 Claim、SSH 公钥 Claim | 不支持 | 支持 | 支持 |
| 用户同步、认证源启用状态 | 后台确认 | 后台确认 | 后台确认 |
...
这两个环境变量。`gitea@2` 会按配置追加受目标版本支持的 `--icon-url`、Claim、组映射和布尔参数。
