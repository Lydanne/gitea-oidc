...
应用开发者在本地校验 X OIDC 连接描述、诊断 OIDC discovery，并为 Node.js 项目生成
...
- connection 文件必须符合 `@x-oidc/contracts` 的严格 schema。

在 workspace 内构建和测试：

```bash
pnpm --filter @x-oidc/cli build
pnpm --filter @x-oidc/cli test
pnpm --filter @x-oidc/cli typecheck
```
...
```bash
x-oidc config validate ./connection.json
```
...
```bash
x-oidc config print ./connection.json --redact
```
...
```bash
x-oidc doctor ./connection.json
x-oidc doctor ./connection.json --timeout 10000
x-oidc doctor ./connection.json --allow-private-network
```
...
1. 检测到 `@nestjs/common` 时推荐 `@x-oidc/nestjs`；
2. 检测到 `fastify` 时推荐 `@x-oidc/fastify`；
3. 检测到 `express` 时推荐 `@x-oidc/express`；
4. 没有已支持框架时推荐 `@x-oidc/node`。
...
写文件前失败，并提示先在管理系统补充回调，或改用 `@x-oidc/node` 自行适配路由。

默认命令只显示推荐包和脱敏的 `.env` 预览，不读取凭据，也不写文件：

```bash
x-oidc init ./connection.json
```
...
X_OIDC_CLIENT_ID='your-client-id'
X_OIDC_REDIRECT_URI='https://app.example.com/oidc/callback'
X_OIDC_SCOPES='openid profile email'
X_OIDC_CLIENT_SECRET='[REDACTED: 通过安全输入提供]'
```

public Client 不生成 `X_OIDC_CLIENT_SECRET`。

生成文件只面向 dotenv 库或 Node.js `--env-file`。不要执行
`source .env.x-oidc` 或 `. .env.x-oidc`；CLI 会在文件头写入相同警告，并对无法无损
round-trip 的值拒绝写入。
...
只有添加 `--write`、CLI 证明 `.env.x-oidc` 已被 Git 忽略，并在交互式 TTY 中输入
`yes` 后，CLI 才会创建文件。confidential Client 默认在确认后通过隐藏输入读取 Secret：

```bash
x-oidc init ./connection.json --write
```
...
```bash
x-oidc init ./connection.json --write --credential-file ./credential.json
```
...
- dry-run 不读取凭据文件；
- 输出和错误信息不会回显 Secret；
- 在支持 POSIX owner-only mode 的平台上，`.env.x-oidc` 使用独占创建和 `0600` 权限，
  已存在时绝不覆盖；
