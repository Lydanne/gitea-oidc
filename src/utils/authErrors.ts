...
import { AuthErrorCode, type AuthError } from '../types/auth';
...
export const AuthErrors = {
  ...
  missingParameter: (parameterNames: string[]) =>
    createAuthError(AuthErrorCode.MISSING_PARAMETER, `Missing required parameters: ${parameterNames.join(', ')}`, {
  ...
  tokenExchangeFailed: (cause?: Error) =>
    createAuthError(AuthErrorCode.TOKEN_EXCHANGE_FAILED, 'Failed to exchange authorization code for token', {
      cause,
      retryable: true,
      suggestedAction: '请重新尝试登录',
    }),
  ...
  providerNotFound: (provider: string) =>
    createAuthError(AuthErrorCode.PROVIDER_NOT_FOUND, `Authentication provider not found: ${provider}`, {
      details: { provider },
  ...
  providerDisabled: (provider: string) =>
    createAuthError(AuthErrorCode.PROVIDER_DISABLED, `Authentication provider is disabled: ${provider}`, {
      details: { provider },
